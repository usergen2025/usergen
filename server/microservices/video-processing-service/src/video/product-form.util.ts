import { PresentationProfile } from './product-presentation.types';
...
/** Forms that have a standard retail display prop. */
...
export function inferProductForm(ctx: ProductFormContext): ProductForm {
  ...
  // Bracelet BEFORE necklace/chain — "chain bracelet" must not become necklace
  if (
...
/**
 * Retail display holder prompt for display_mannequin — matched to product form only.
 * Unknown/generic: neutral surface, no wrong-category prop.
 */
...
/** Strong lock suffix injected into every PRODUCT_ONLY image prompt. */
...
/** Strip or flag conflicting keywords from GPT scene prompts. */
