...
/**
 * Service to get public URLs for local audio files
 * 
 * Priority order:
...
export class PublicUrlService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    const servicePort = this.configService.get<number>('SERVICE_PORT', 9002);
    ...
    if (this.unifiedStorage.isGcsAvailable()) {
      console.log(`[PublicUrlService] ✅ GCS storage initialized for voice-audio-service (bucket: ${gcsConfig.bucketName})`);
    } else {
   ...
   * @param localUrl - Relative URL path (e.g., /uploads/audio/user123/file.mp3)
   * @returns Public URL that can be used by external services
   */
   ...
   * @param subPath - Sub-path within service folder (e.g., "audio/user123")
  ...
  async uploadFromBuffer(
    ...
    if (result.gcsUploaded) {
      console.log(`[PublicUrlService] ✅ Audio file uploaded to GCS: ${result.gcsUrl}`);
    } else {
      console.log(`[PublicUrlService] Audio file saved locally: ${result.localPath}`);
    }
