...
async function bootstrap() {
  ...
  const port = configService.get<number>('SERVICE_PORT', 9009);
  ...
  const config = new DocumentBuilder()
    .setTitle('Media Management Service API')
    .setDescription('Media file management API for UserGen.ai platform')
    .setVersion('1.0.0')
    .addBearerAuth()
  ...
  app.getHttpAdapter().get('/health', (req, res) => {
    res.status(200).json({ status: 'ok', service: 'media-management-service', timestamp: new Date().toISOString(), uptime: process.uptime() });
  });

  await app.listen(port);
  console.log(`🚀 Media Management Service running on port ${port}`);
