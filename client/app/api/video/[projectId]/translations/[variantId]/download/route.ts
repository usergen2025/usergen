...
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ projectId: string; variantId: string }> },
) {
  ...
  try {
    const upstream = await fetch(
      `${VIDEO_SERVICE_URL}/video-projects/${encodeURIComponent(projectId)}/translations/${encodeURIComponent(variantId)}/download${qs}`,
      {
        headers: {
    ...
    console.error('[Translation download proxy]', message);
    return NextResponse.json(
